...
export const NetworkSelect = ({ networkBranch }) => {
  ...
  for (const _netId in constants.NETWORKS) {
    ...
    if (constants.NETWORKS[_netId].NAME.toLowerCase() === networkBranch) {
    ...
    <div className={`NetworkSelect nl-NavigationLinks_Link opacityFull`}>
      <div className={`NetworkSelect_Top`}>
        <svg className={`nl-IconNetwork`} xmlns="http://www.w3.org/2000/svg" width="18" height="18">
          <path
            className={`nl-IconNetwork_Path nl-IconNetwork_Path-${networkBranch}`}
        ...
        <span className={`nl-NavigationLinks_Text nl-NavigationLinks_Text-${networkBranch}`}>
